...
`mark6` 是当前这条“离线 pcap 回放 + 模拟 RSS + 首包代价预测 + 异构核调度”实验线。
...
`mark6` 现在不是在 `mark3` 上简单加一个端口表判断，而是把原来 `flow -> worker` 的软件 RSS 阶段，换成了一个真正的“调度层”。
...
- `/home/zync/WORKSPACE/ndpi_speed/mark6/offline_costs/lookup_table.json`
...
这是 `mark6` 和 `mark3` 最大的行为差异之一。

当前 `mark6` 的调度层不再用 reader 侧的 quick hash 做 `flow -> worker` 粘性映射，而是用：
...
当前默认 `mark6/offline_costs/cost_profile.csv` 先用 P-core profiling 初始化，E-core 行暂时镜像 P-core 行：
...
换句话说，当前 `mark6` 已经是一个功能正确的实验平台，但还不是一个已经调优完成的异构调度器。
