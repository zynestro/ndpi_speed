# mark6 offline cost profiles

This directory is the default offline-rule directory used by `mark6`.
...
If only one core type is present, `mark6` copies those values to the missing core type. That makes it possible to start with P-core profiling and later replace the `E,*` rows after E-core profiling is ready.
...
```text
mark6/offline_costs/lookup_table.json
