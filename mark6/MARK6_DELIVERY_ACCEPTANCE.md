...
本文档用于验收当前 `mark6` benchmark 的端到端运行流程，重点确认：
...
- `ndpiBenchmarkMark6`：默认 cost-aware-jsw 策略；
- `ndpiBenchmarkMark6Hash`：`MARK6_DISPATCH_HASH_ONLY=1` 的 hash-only 对照策略。

核心代码核实：

- 构建源文件列表：`mark6/CMakeLists.txt:43-60`
- 默认优化构建：`mark6/CMakeLists.txt:15-19`
- mark6 热路径编译开关：`mark6/CMakeLists.txt:70-74`
- 两个 target：`mark6/CMakeLists.txt:88-97`

验收点：

- `cmake --build mark6/build -j` 能同时构建两个 target；
- Release 模式使用 `-O3`；
...
- benchmark 主入口：`mark6/src/main.c:523-760`
- worker 线程启动：`mark6/src/main.c:653-658`
- reader 线程启动与 join：`mark6/src/main.c:697-705`
- reader 主控流程：`mark6/src/reader.c:528-614`
- dispatcher 线程入口：`mark6/src/reader.c:415-526`
- worker 线程入口：`mark6/src/worker.c:384-409`
...
- 默认常量：`mark6/src/main.c:13-19`
- 策略名称宏：`mark6/src/main.c:21-25`
- 参数解析入口：`mark6/src/main.c:421-520`
- 默认 worker core list：`mark6/src/main.c:358-370`
- dispatcher 默认 core list：`mark6/src/main.c:511-518`

### 4.3 验收点

- 不传 `-c` 且 `-n=16` 时，worker 自动使用 8 个 P 核 + 8 个 E 核；
- 不传 `-d` 时，dispatcher 自动使用 core 16-23；
- `MARK6_DISPATCH_HASH_ONLY` target 输出策略名为 `hash-only`，否则为 `cost-aware-jsw`。
...
- nDPI 全局初始化：`mark6/src/main.c:533-539`
- lookup table 加载：`mark6/src/main.c:541-547`
- cost profile 加载：`mark6/src/main.c:549-556`
- worker runtime state 分配：`mark6/src/main.c:372-389`
- core type 判定：`mark6/src/dispatch.c:46-53`
- runtime state 字段定义：`mark6/include/ndpi_benchmark.h:409-419`
...
- worker 数组创建：`mark6/src/main.c:573-580`
- worker 字段填充：`mark6/src/main.c:581-586`
- worker 队列创建：`mark6/src/main.c:588-600`
- worker nDPI 初始化调用：`mark6/src/main.c:618-620`
- `init_worker_ndpi()`：`mark6/src/worker.c:70-93`
- 队列结构定义：`mark6/include/ndpi_benchmark.h:56-77`
- 队列创建：`mark6/include/ndpi_benchmark.h:112-130`
...
- 当前 mark6 队列使用 reference 模式：`queue_packet_t.data` 指向预加载包数据，不再在 enqueue 阶段复制 payload。
...
- main 创建 dispatch context：`mark6/src/main.c:623-625`
- dispatch context 结构：`mark6/src/dispatch.c:27-37`
- shard 结构：`mark6/src/dispatch.c:16-25`
- shard 分配与初始化：`mark6/src/dispatch.c:162-196`
- lookup 中按 shard_id 取 shard：`mark6/src/dispatch.c:208-220`
- 热路径无 mutex 注释与实现：`mark6/src/dispatch.c:221-227`
...
- worker 线程创建：`mark6/src/main.c:653-658`
- reader context 初始化：`mark6/src/main.c:660-685`
- wall time / cycles 开始计时：`mark6/src/main.c:689-695`
- reader 线程创建：`mark6/src/main.c:697-700`
- reader join 与 worker join：`mark6/src/main.c:702-707`
...
- 预处理入口：`mark6/src/reader.c:230-380`
- `pcap_open_offline()`：`mark6/src/reader.c:238-243`
- `pcap_next_ex()` 计时：`mark6/src/reader.c:262-266`
- 标准化调用：`mark6/src/reader.c:288-295`
- parse / flow key / hash：`mark6/src/reader.c:306-336`
- dispatcher id 计算：`mark6/src/reader.c:338-347`
- packet store：`mark6/src/reader.c:349-364`
- `dispatch_packet_t` 字段：`mark6/src/reader.c:21-34`
- `append_packet()`：`mark6/src/reader.c:128-168`
- Ethernet 标准化实现：`mark6/src/packet_parser.c:28-120`
- Ethernet parse 实现：`mark6/src/packet_parser.c:204-260`
...
- `build_dispatch_schedule()`：`mark6/src/reader.c:170-228`
- counts 统计：`mark6/src/reader.c:173-183`
- offsets 构建：`mark6/src/reader.c:185-193`
- indices 填充：`mark6/src/reader.c:195-220`
- reader 中调用：`mark6/src/reader.c:559-576`
...
- dispatcher 创建：`mark6/src/reader.c:578-600`
- dispatcher join：`mark6/src/reader.c:602-604`
- dispatcher 入口：`mark6/src/reader.c:415-526`
- dispatcher 绑核：`mark6/src/reader.c:424-426`
- dispatcher 范围：`mark6/src/reader.c:437-449`
- flow->worker 调用：`mark6/src/reader.c:452-467`
- batch item 填充：`mark6/src/reader.c:473-487`
- 末尾 flush：`mark6/src/reader.c:515-521`
- dispatch 统计合并：`mark6/src/reader.c:383-391`
...
- lookup 函数签名：`mark6/src/dispatch.c:208-214`
- shard 选择：`mark6/src/dispatch.c:219-220`
- rehash 判断：`mark6/src/dispatch.c:229-231`
- 开放寻址查找：`mark6/src/dispatch.c:233-292`
- 新 flow cost bucket：`mark6/src/dispatch.c:240-247`
- hash-only 分支：`mark6/src/dispatch.c:251-252`
- cost-aware 分支：`mark6/src/dispatch.c:253-254`
- 新 flow 写表与统计：`mark6/src/dispatch.c:257-277`
- 已有 flow 命中：`mark6/src/dispatch.c:281-288`
- cost-aware worker 选择：`mark6/src/dispatch.c:75-113`
- queue depth / pending cost 读取：`mark6/src/dispatch.c:82-96`
...
- 编译开关：`mark6/CMakeLists.txt:70-74`
- `queue_packet_t` 引用模式字段：`mark6/include/ndpi_benchmark.h:56-66`
- `packet_queue_push_batch()`：`mark6/include/ndpi_benchmark.h:180-221`
- queue `head` 发布：`mark6/include/ndpi_benchmark.h:218-219`
- worker `peek`：`mark6/include/ndpi_benchmark.h:224-233`
- worker `consume`：`mark6/include/ndpi_benchmark.h:236-239`
- queue finish：`mark6/include/ndpi_benchmark.h:248-250`
- 预加载包延迟清理：`mark6/src/main.c:702-707`
- reader 不在成功路径清理预加载包：`mark6/src/reader.c:606-613`
...
- worker 线程循环：`mark6/src/worker.c:384-409`
- queue peek：`mark6/include/ndpi_benchmark.h:224-233`
- worker 单包入口：`mark6/src/worker.c:184-379`
- parse：`mark6/src/worker.c:193-206`
- flow key：`mark6/src/worker.c:208-220`
- flow lookup/create：`mark6/src/worker.c:244-258`
- 新 flow 初始化：`mark6/src/worker.c:260-283`
- 方向和流量统计：`mark6/src/worker.c:286-303`
- nDPI 调用：`mark6/src/worker.c:312-324`
- 协议边沿检查：`mark6/src/worker.c:332-352`
- worker 包/字节统计：`mark6/src/worker.c:354-359`
- queue depth retire：`mark6/src/worker.c:397-404`
- queue consume：`mark6/src/worker.c:405`
...
- dispatcher join：`mark6/src/reader.c:602-604`
- queue finish：`mark6/src/reader.c:606-609`
- reader finalize timers：`mark6/src/reader.c:611-612`
- main join：`mark6/src/main.c:702-707`
- reader 预加载包清理：`mark6/src/reader.c:123-126`
- 最终资源释放：`mark6/src/main.c:748-757`
- worker cleanup：`mark6/src/worker.c:98-122`
...
- `print_benchmark_results()`：`mark6/src/main.c:27-286`
- wall time 输出：`mark6/src/main.c:94-103`
- dispatch 输出：`mark6/src/main.c:104-107`
- process 输出：`mark6/src/main.c:108-117`
- total packets / bytes / throughput：`mark6/src/main.c:124-129`
- 协议识别校验：`mark6/src/main.c:131-140`
- per-worker 原始统计：`mark6/src/main.c:158-202`
- per-worker load details：`mark6/src/main.c:204-233`
- P/E summary：`mark6/src/main.c:235-260`
- imbalance hints：`mark6/src/main.c:262-278`
- dispatch summary：`mark6/src/main.c:392-419`
- main 调用统计输出：`mark6/src/main.c:732-746`
...
- shard dispatch：`mark6/src/dispatch.c:16-37`
- lookup 传入 `key_hash`：`mark6/include/benchmark_internal.h:160-166`
- reader 调用传入 `pkt->flow_hash`：`mark6/src/reader.c:456-462`
- batch enqueue：`mark6/src/reader.c:428-430`、`mark6/src/reader.c:473-487`
- batch flush：`mark6/src/reader.c:515-521`
- queue reference mode：`mark6/CMakeLists.txt:70-74`
- 延迟清理：`mark6/src/main.c:702-707`
...
```bash
cmake --build mark6/build -j
./mark6/build/ndpiBenchmarkMark6 -i mark1/test.pcap -q
./mark6/build/ndpiBenchmarkMark6Hash -i mark1/test.pcap -q
./mark6/build/ndpiBenchmarkMark6 -i input/Monday-WorkingHours.pcap -q
```
