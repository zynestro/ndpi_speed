...
这份文档按“先看整体，再看一次运行的时间顺序”的方式解释 `mark6`。读完之后，你应该能知道：
...
`mark6` 是一个离线 pcap benchmark，不是真实网卡收包程序。它把 pcap 文件预加载到内存，然后模拟一条多线程处理流水线：
...
```text
ndpiBenchmarkMark6      cost-aware-jsw 调度策略
ndpiBenchmarkMark6Hash  hash-only baseline
```

它们使用同一套源码，只是 `ndpiBenchmarkMark6Hash` 编译时打开了 `MARK6_DISPATCH_HASH_ONLY`。
...
- `ndpiBenchmarkMark6Hash` 额外定义 `MARK6_DISPATCH_HASH_ONLY=1`。
...
```text
mark6/offline_costs/cost_profile.csv
```
...
注意：有些源码注释里还写着 `mark3`，这是历史继承注释；当前目录里的代码实际是 `mark6`。
...
- 现在 `mark6` 预处理分 dispatcher 主要直接用 `flow_hash % num_dispatchers`；
...
它是 `mark6` 调度层的核心状态，负责保证同一个 flow 后续继续回到同一个 worker。
...
```bash
./mark6/build/ndpiBenchmarkMark6 \
...
```bash
cmake -S mark6 -B mark6/build -DNDPI_PREFIX=$HOME/ndpi-install
...
```bash
./mark6/build/ndpiBenchmarkMark6Hash \
  -i input/Monday-WorkingHours.pcap
```
...
./mark6/build/ndpiBenchmarkMark6 \
  -i input/Monday-WorkingHours.pcap \
  -m mark6/offline_costs/lookup_table.json
```
...
./mark6/build/ndpiBenchmarkMark6 \
  -i input/Monday-WorkingHours.pcap \
  -C mark6/offline_costs/cost_profile.csv
```
