...
bool normalize_to_ethernet(int linktype,
  ...
  /* Linux cooked capture v1: 16-byte SLL header，协议号在最后 2 字节。 */
#ifdef DLT_LINUX_SLL
  if (linktype == DLT_LINUX_SLL) {
    if (caplen < 16) return false;

    uint16_t ether_type = (uint16_t)((data[14] << 8) | data[15]);
    uint16_t payload_caplen = (uint16_t)(caplen - 16);
    uint16_t new_caplen = (uint16_t)(payload_caplen + ETH_HDR_LEN);
    if (new_caplen > scratch_len) return false;
    ...
    scratch[13] = (uint8_t)(ether_type & 0xFF);
    if (payload_caplen > 0) {
      memcpy(scratch + ETH_HDR_LEN, data + 16, payload_caplen);
    }
...
parse_result_t parse_ethernet_frame(const uint8_t *data, uint16_t caplen, parsed_packet_t *out) {
  ...
  if (ether_type == ETHERTYPE_IPV4) {
    ...
    if (out->l4_proto == IPPROTO_TCP) {
      ...
      uint16_t tcp_hdr_len = (uint16_t)(tcp->th_off * 4);
      if (tcp_hdr_len >= sizeof(struct tcphdr) && caplen >= l4_off + tcp_hdr_len) {
        out->payload = data + l4_off + tcp_hdr_len;
        out->payload_len = (uint16_t)(caplen - l4_off - tcp_hdr_len);
      }
      ...
      out->payload = data + l4_off + sizeof(struct udphdr);
      out->payload_len = (uint16_t)(caplen - l4_off - sizeof(struct udphdr));
    } else {
      out->src_port = 0;
      out->dst_port = 0;
      out->payload = NULL;
