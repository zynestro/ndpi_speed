...
parse_result_t parse_ethernet_frame(const uint8_t *data, uint16_t caplen, parsed_packet_t *out) {
  ...
  if (ether_type == ETHERTYPE_IPV4) {
    ...
    if (out->l4_proto == IPPROTO_TCP) {
      ...
      uint8_t tcp_hdr_len = (uint8_t)(tcp->th_off * 4);
      if (tcp_hdr_len < sizeof(struct tcphdr)) return PARSE_UNSUPPORTED;
      if (caplen < l4_off + tcp_hdr_len) return PARSE_TRUNCATED;
