...
def run_benchmark(binary_path, pcap_file, num_workers, quiet=True):
    ...
    try:
        ...
        match = re.search(r'Read enqueue hash/rss:\s+([\d.]+)\s+seconds', output)
        if match:
            metrics['read_enqueue_hash_sec'] = float(match.group(1))

        match = re.search(r'Read enqueue queue_wait:\s+([\d.]+)\s+seconds', output)
        if match:
            metrics['read_enqueue_queue_wait_sec'] = float(match.group(1))

        match = re.search(r'Read enqueue queue_write:\s+([\d.]+)\s+seconds', output)
        if match:
            metrics['read_enqueue_queue_write_sec'] = float(match.group(1))
...
def run_sweep(binary_path, pcap_file, worker_range, output_dir):
    results = {
        ...
        'read_pcap_sec': [],
        'read_enqueue_sec': [],
        'read_enqueue_hash_sec': [],
        'read_enqueue_queue_wait_sec': [],
        'read_enqueue_queue_write_sec': [],
        'proc_parse_sec': [],
        'proc_flow_sec': [],
        'proc_ndpi_sec': [],
        'proc_other_sec': [],
    ...
    for workers in worker_range:
        ...
        if metrics and 'bandwidth_gbps' in metrics:
            ...
            rq_hash = metrics.get('read_enqueue_hash_sec', 0)
            rq_wait = metrics.get('read_enqueue_queue_wait_sec', 0)
            rq_write = metrics.get('read_enqueue_queue_write_sec', 0)
            ...
            results['read_enqueue_hash_sec'].append(rq_hash)
            results['read_enqueue_queue_wait_sec'].append(rq_wait)
            results['read_enqueue_queue_write_sec'].append(rq_write)
...
def plot_results(results, output_dir, worker_range, binary_path, pcap_file, timestamp):
    ...
    if workers:
        ...
        ax.plot(workers, results['read_enqueue_sec'],
                marker='s', linewidth=1.8, markersize=5, linestyle='--', label='enqueue total')
        ax.plot(workers, results['read_enqueue_hash_sec'],
                marker='^', linewidth=1.8, markersize=5, label='enqueue hash/rss')
        ax.plot(workers, results['read_enqueue_queue_wait_sec'],
                marker='d', linewidth=1.8, markersize=5, label='enqueue queue_wait')
        ax.plot(workers, results['read_enqueue_queue_write_sec'],
                marker='x', linewidth=1.8, markersize=5, label='enqueue queue_write')
    ax.set_xlabel('Workers')
    ax.set_ylabel('Time (s)')
...
def save_csv(results, output_dir, timestamp):
    ...
    headers = [
        ...
        'read_pcap_sec', 'read_enqueue_sec', 'read_enqueue_hash_sec',
        'read_enqueue_queue_wait_sec', 'read_enqueue_queue_write_sec',
        'proc_parse_sec', 'proc_flow_sec',
        'proc_ndpi_sec', 'proc_other_sec'
    ] + core_headers
    ...
    with open(csv_path, 'w', newline='') as f:
        ...
        for i in range(len(results['workers'])):
            row = [
                ...
                f"{results['read_enqueue_hash_sec'][i]:.6f}",
                f"{results['read_enqueue_queue_wait_sec'][i]:.6f}",
                f"{results['read_enqueue_queue_write_sec'][i]:.6f}",
