...
/* 入队（生产者调用）- 阻塞直到有空间（可选统计等待/写入开销） */
static inline bool packet_queue_push_timed(packet_queue_t *q,
                                           const uint8_t *data, uint16_t caplen,
                                           uint16_t wirelen, uint64_t timestamp_us,
                                           uint64_t *wait_us, uint64_t *write_us) {
  uint64_t wait_delta_us = 0;
  uint32_t head = atomic_load_explicit(&q->head, memory_order_relaxed);
  uint32_t tail = atomic_load_explicit(&q->tail, memory_order_acquire);
  uint64_t wait_begin_us = 0;
  while ((head - tail) >= q->capacity) {
    if (wait_begin_us == 0) wait_begin_us = get_time_us();
    if (atomic_load_explicit(&q->finished, memory_order_relaxed)) {
      if (wait_begin_us != 0) wait_delta_us += (get_time_us() - wait_begin_us);
      if (wait_us) *wait_us += wait_delta_us;
      return false;
    }
    packet_queue_pause();
  ...
  uint64_t write_begin_us = get_time_us();
  ...
  if (wait_us) *wait_us += wait_delta_us;
  if (write_us) *write_us += (get_time_us() - write_begin_us);
  return true;
}
...
static inline bool packet_queue_push(packet_queue_t *q,
                                     ...
                                     uint64_t timestamp_us) {
  return packet_queue_push_timed(q, data, caplen, wirelen, timestamp_us, NULL, NULL);
}
...
/* 入队（生产者调用）- 批量提交版本（可选统计等待/写入开销） */
static inline bool packet_queue_push_cached_timed(packet_queue_t *q,
...
static inline bool packet_queue_push_cached(packet_queue_t *q,
                                            ...
                                            const uint8_t *data, uint16_t caplen,
                                            uint16_t wirelen, uint64_t timestamp_us) {
  return packet_queue_push_cached_timed(q, p, data, caplen, wirelen, timestamp_us, NULL, NULL);
}
...
typedef struct {
  ...
  uint64_t flow_time_us;
  uint64_t ndpi_time_us;
  uint64_t other_time_us;
