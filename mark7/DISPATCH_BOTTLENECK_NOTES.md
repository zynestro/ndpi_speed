# mark7 Dispatch Bottleneck Notes
...
Queue slots reserve `MAX_PACKET_SIZE` bytes per packet. In mark7 the preloaded
...
3. Make the queue packet buffer size configurable and set mark7 to a 2048-byte
...
- mark7 builds with batched enqueue enabled.
...
- Queue entries can carry packet data by reference. mark7 enables this mode, so
